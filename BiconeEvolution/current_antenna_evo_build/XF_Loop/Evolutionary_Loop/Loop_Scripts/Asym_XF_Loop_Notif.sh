...
RunName='AraSim_Polarity_Fix_2021_03_19'      ## This is the name of the run. You need to make a unique name each time you run.
...
do
	...
	then
		...
		./ssPermissionCheck.sh $RunName $state
	...
	then
		...
		./Part_D1_AraSeed_Notif.sh $gen $NPOP $WorkingDir $AraSimExec $exp $NNT $RunName $Seeds $DEBUG_MODE
		state=6
	...
	if [ $state -eq 6 ]
	then
		#./Part_D2_AraSeed.sh $gen $NPOP $WorkingDir $RunName $Seeds $AraSimExec
		./Part_D2_AraSeed_Notif.sh $gen $NPOP $WorkingDir $RunName $Seeds $AraSimExec
		state=7
		./SaveState_Prototype.sh $gen $state $RunName $indiv
		./ssPermissionCheck.sh $RunName $state
	...
	then
		...
		./Part_E_Asym.sh $gen $NPOP $WorkingDir $RunName $ScaleFactor $AntennaRadii $indiv $Seeds $GeoFactor $AraSimExec $XFProj $NSECTIONS $SEPARATION
