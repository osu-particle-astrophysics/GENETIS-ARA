...
do
	...
	do
		...
		then
			...
			if grep "segmentation violation" ../${gen}_AraSim_Errors/${current_file}.error || grep "DATA_LIKE_OUTPUT" ../${gen}_AraSim_Errors/${current_file}.error || grep "CANCELLED" ../${gen}_AraSim_Errors/${current_file}.error 
			then
