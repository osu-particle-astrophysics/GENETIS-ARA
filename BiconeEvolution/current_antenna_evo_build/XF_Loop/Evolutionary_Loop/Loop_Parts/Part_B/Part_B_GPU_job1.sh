...
if [ ${gen} -eq 0 ]
then
	mkdir -m775 XF_Outputs
	mkdir -m775 XF_Errors
fi
...
sbatch --array=1-${NPOP}%${batch_size} --mem-per-gpu=178gb --export=ALL,WorkingDir=$WorkingDir,RunName=$RunName,XmacrosDir=$XmacrosDir,XFProj=$XFProj,NPOP=$NPOP,indiv=$individual_number,indiv_dir=$indiv_dir,gen=${gen} --job-name=${RunName} Batch_Jobs/GPU_XF_Job.sh >> ~/XF_Error_Log/Part_B.log
