...
int main(int argc, char** argv)
{
	...
	double* antennaQuadratics1 = new double[NPOP];
	double* antennaLinears1 = new double[NPOP];
	double* antennaOuterRadii1 = new double[NPOP];
	double* antennaLengths2 = new double[NPOP];
	double* antennaQuadratics2 = new double[NPOP];
	double* antennaLinears2 = new double[NPOP];
		...
		for (int i = 0; i < NPOP * NSECTIONS; i+=NSECTIONS) {
			for (int j = 0; j < NSECTIONS; j++) {
				...
				if (j == 0) {
		  			...
		  			antennaQuadratics1[i / NSECTIONS] = stod(row[2]);
					antennaLinears1[i / NSECTIONS] = stod(row[3]);
		  			antennaOuterRadii1[i / NSECTIONS] = antennaRadii[i / NSECTIONS] + antennaQuadratics1[i / NSECTIONS]*antennaLengths1[i / NSECTIONS]*antennaLengths1[i / NSECTIONS] + antennaLinears1[i / NSECTIONS]*antennaLengths1[i / NSECTIONS];
		  			cout << "Calculated Outer Radii: " << antennaOuterRadii1[i / NSECTIONS] << endl;
				}
				else if (j == 1) {
					antennaLengths2[i / NSECTIONS] = stod(row[1]);
					antennaQuadratics2[i / NSECTIONS] = stod(row[2]);
					antennaLinears2[i / NSECTIONS] = stod(row[3]);
					antennaOuterRadii2[i / NSECTIONS] = antennaRadii[i / NSECTIONS] + antennaQuadratics2[i / NSECTIONS]*antennaLengths2[i / NSECTIONS]*antennaLengths2[i / NSECTIONS] + antennaLinears2[i / NSECTIONS]*antennaLengths2[i / NSECTIONS];;
	...
	delete[] antennaQuadratics1;
	delete[] antennaLinears1;
	delete[] antennaOuterRadii1;
	delete[] antennaLengths2;
	delete[] antennaQuadratics2;
	delete[] antennaLinears2;
