...
float min_B = 0;//-1;//-0.1; 
float max_B = max_radius/min_length;//0.8; // They represent slopes (what are the axes?)
			...
			for(int i=0;i<NPOP;i++)
			{
				for(int j=0;j<NSECTIONS;j++)
				{
					...
					// 		IF the parabola is concave down, check the final value f(l) > 0
                                        if(max_outer_radius > end_point && end_point >= 0.0)
                                            {
	...
	for(int k=0; k<NVARS; k++)
	  {
	    ...
	    float R_2= varOutput[i+1+reproduction_no][j][0];
            ...
            if(max_outer_radius > end_point_1 && end_point_1 >= 0.0 && max_outer_radius > end_point_2 && end_point_2 >= 0.0){ 
              j=j;
