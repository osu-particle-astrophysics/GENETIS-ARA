...
float min_A = 0.0; //-1.0;//-0.008;
float max_A = 0.0; //1.0;//-0.002;

// how did they decide on these values?
float min_B = 0; //-1;//-0.1; 
float max_B = max_outer_radius/min_length; //1;//0.8; // They represent slopes (what are the axes?)
			...
			for(int i=0;i<NPOP;i++)
			{
				for(int j=0;j<NSECTIONS;j++)
				{
					...
					for(int k=0;k<NVARS;k++)
					{
						...
						{
							float A = 0.0; //distribution_A(generator);
							generator_file << generator << endl;	
							varOutput[i][j][k]= A;
						}
					...
					if(a == 0.0 && max_outer_radius > end_point && end_point >=0.0)
					  {
					    intersect_condition =1; // No intersection
                                        ...
                                        else if( a != 0.0 && max_outer_radius > end_point && end_point >= 0.0)
                                            {
                                                intersect_condition = 1; // No intersection
                                            }
                                        else{
	...
	for(int k=0; k<NVARS; k++)
	  {
		...
		j=j;
            ...
            else if(a_1 !=0.0 && max_outer_radius > end_point_1 && end_point_1 >= 0.0 && a_2 != 0 && max_outer_radius > end_point_2 && end_point_2 >= 0.0)
	      { 
              j=j;
            ...
            else if(a_2 == 0.0 && max_outer_radius > end_point_2 && end_point_2 >= 0.0 && a_1 !=0.0 && max_outer_radius > end_point_1 && end_point_1 >= 0.0)
              {
              j=j;
              }
...
void mutation(vector<vector<vector<float> > > & varOutput, int reproduction_no, int crossover_no, float max_length, float max_radius, float max_seperation, float max_outer_radius, float max_A, float max_B)
{
  ...
  for(int i=reproduction_no+crossover_no; i< varOutput.size(); i++)
      ...
      for(int j=0; j<NSECTIONS; j++)
	{
	  ...
	  varOutput[i][j][2] = 0.0; //a_mut(generator);
